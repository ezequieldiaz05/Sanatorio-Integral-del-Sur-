...
public class ControladorOrdenes {
    ...
    // Agrega una nueva orden de compra al sistema. Valida que no exista otra con el mismo numero.
    public Boolean agregarOrdenCompra(OrdenCompra orden) {
        ...
        if (existeOrdenCompra(orden.getNroOC())) {
            System.err.println("Error: Ya existe una orden con número " + orden.getNroOC());
            return false;
        }

        ordenesCompra.add(orden);
        System.out.println("Orden de compra agregada: " + orden.getNroOC());
        return true;
    }
